...
struct machinfo *lookup(char *host)
{
    ...
    if (!machine_info) {
        if ((machine_number = Sdomain(host)) < 0) return(0);  /* initiate domain name lookup */

        while (machine_info==NULL) {                /* wait for DOMOK or DOMFAIL event */
            switch(lgetevent(USERCLASS,&what,&dat)) {
                case DOMFAIL: return(0);                             /* lookup failed, return 0 */
                case DOMOK:   machine_info=Slooknum(machine_number); /* get pointer to machine record */
                default: break;
            }
        }
...
#define BUFF_SIZE 1024
{
    ...
    while (buff_ptr < (buff + len1)) {
        len2 = netwrite(connection_id, buff_ptr, len1-(buff_ptr - buff));
        checkerr();
        Stask();
